...
<head>
    ...
    <meta name="description" content="موقع لمساعدة طلاب قسم الميكاترونكس في جامعة الزقازيق الأهلية، يوفر موارد دراسية، خطط، حاسبة معدل تراكمي، والمزيد لتسهيل الرحلة التعليمية.">

        <!-- 
      *** START: EDITS FOR PWA / FULL-SCREEN APP MODE ***
    -->
    
    <!-- iOS (iPhone/Safari) Meta Tags -->
    <!-- This makes the app launch in full-screen (no Safari UI) *when added to the Home Screen*. -->
    <meta name="apple-mobile-web-app-capable" content="yes">
    <!-- This sets the status bar style. 'black-translucent' lets content flow under it. 'default' or 'black' are other options. -->
    <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">

    <!-- Android (Chrome) Meta Tags -->
    <!-- This also enables the "Add to Home Screen" app-like experience. -->
    <meta name="mobile-web-app-capable" content="yes">
    <!-- This colors the browser's top bar (and status bar on Android) to match your site's theme. -->
    <meta name="theme-color" content="#1A741A"> 
    
    <!-- *** END: EDITS FOR PWA / FULL-SCREEN APP MODE *** -->
    ...
    <style>
        ...
        /* MODIFICATION: This is the primary (and only) style for the new header now */
        .new-main-header {
            position: fixed;
            top: 0;
            left: 0;
            ...
            background: linear-gradient(
                135deg,
                rgba(255,255,255,0.65), /* MOD: Made less transparent */
                rgba(255,255,255,0.3)  /* MOD: Made less transparent */
            );
    ...
    max-height: min(420px, 65vh); /* more than 6-10 messages, scroll inside section only */
    overflow-y: auto;
    scroll-behavior: smooth;
...
.notifications-list.single-notification {
    max-height: none;
    overflow: visible;
}
...
.notification-card h3 {
        ...
        .notification-actions .general-button {
            flex: 1 1 140px;
        }
...
nimation properties added via .animated-section .section-header */
        ...
        .site-footer {
            ...
            margin-top: auto;
            text-align: center;
            /* MODIFICATION: Removed border-radius */
    ...
    align-items: center;
    gap: 0.5rem;
    /* MODIFICATION: Adjusted padding and border-radius */
...
.visitor-counter #visitor-count {
    ...
    color: #ffffff;
        ...
        main {
            margin-top: 0;
            /* MODIFICATION: Added padding to avoid bottom nav overlap */
            padding-bottom: 80px;
        }
        .main-nav {
            position: fixed;
...
@media (max-width: 767px) {
  ...
  body {
    font-size: 0.95rem;
    /* MODIFICATION: Removed bottom padding, moved to main */
    padding-bottom: 0; 
    overflow-x: hidden;
...
/* MOD: Added animation for notification cards */
...
body {
    min-height: calc(var(--vh, 1vh) * 100);
        ...
        .info-card.notification-card {
            ...
            text-align: right;
            /* MODIFICATION: Added smooth fade-in animation */
        ...
        /* Mobile nav smoothness override: remove slideInRight/slideOutRight on phones */
        @media (max-width: 767px) {
          .main-nav {
            animation: none !important;
          }
    ...
    <!-- MODIFICATION: DELETED redundant solid-white header styles -->
    <style>
        /* Desktop nav bar modifications */
        
/* Desktop glossy bottom navbar turned into right-side floating nav */
.bottom-navbar {
    position: fixed;
    right: 24px;
    bottom: 24px;
    ...
    display: flex;
    /* MODIFICATION: Changed direction */
    flex-direction: column; 
    align-items: center;
    gap: 0.75rem;
    ...
    width: 76px;
    height: auto;
    background: rgba(255, 255, 255, 0.75); /* MOD: Made more opaque */
    backdrop-filter: blur(22px) saturate(1.5); /* MOD: Increased blur */
    -webkit-backdrop-filter: blur(22px) saturate(1.5); /* MOD: Increased blur */
    border-radius: 24px;
    box-shadow: 0 20px 40px rgba(0, 0, 0, 0.26);
    border: 1px solid rgba(255, 255, 255, 0.8); /* MOD: More opaque border */
    z-index: 1600;
    transition: transform 0.6s cubic-bezier(0.22, 1, 0.36, 1), opacity 0.3s ease, width 0.35s ease, padding 0.35s ease;
    opacity: 0;
}
...
/* Small arrow pointing towards the page (to the left) */
...
/* === Back To Top === */
#backToTop {
  position: fixed; bottom: 90px; left: 24px; transform: scale(0);
  width: 52px; height: 52px; border-radius: 50%; border: none;
  background: rgba(255,255,255,0.45); backdrop-filter: blur(10px);
  box-shadow: 0 8px 20px rgba(0,0,0,0.2);
  cursor: pointer; z-index: 1600; transition: transform .4s cubic-bezier(.2,.9,.3,1), box-shadow .3s ease, right 0.3s ease, bottom 0.3s ease; /* MOD: Added transitions */
}
#backToTop.visible { transform: scale(1); }

@media (max-width: 767px) {
  #backToTop {
    /* MODIFICATION: Hide on mobile */
    display: none !important;
  }
}
...
@media (min-width: 768px) {
    #backToTop {
        ...
        /* MODIFICATION: Changed position to bottom-left */
        left: 30px;
        right: auto;
...
<div class="bottom-navbar" id="bottomNavbar">
  ...
  <nav>
    ...
    <a href="#notifications-section">الإشعارات</a> <!-- MOD: Added notification link -->
...
<style>
/* MOD: DELETED #i/* MOD: Desktop right-side circular app-style nav */
.bottom-navbar img {
  width: 64px;
  height: 64px;
  border-radius: 50%;
...
.bottom-navbar nav {
  display: flex;
  flex-direction: column;
  /* MODIFICATION: Align links to start */
  align-items: flex-start; 
...
.bottom-navbar nav a {
  ...
  text-decoration: none;
  font-weight: 600;
  /* MODIFICATION: Increased font size and padding */
  font-size: 1.1rem;
  position: relative;
  padding: 0.5rem 1.2rem;
  border-radius: 24px;
  ...
  white-space: nowrap;
  /* MODIFICATION: Make links full width */
  width: 100%;
  box-sizing: border-box; 
}
...
.bottom-navbar nav a::after {
  ...
  box-shadow: 0 0 0 0 rgba(26, 116, 26, 0.35);
  opacity: 0;
...
.bottom-navbar nav a:hover::after {
  opacity: 1;
  box-shadow: 0 0 0 1px rgba(26, 116, 26, 0.25);
}
...
document.addEventListener('DOMContentLoaded', async () => {
    // ================================
    // Per-device notifications (like a phone notification shade)
    // ================================
    const NOTIFICATIONS_API_URL = "/.netlify/functions/visitor-counter?type=notifications";
    const container = document.getElementById("notifications-list");
    const filterButtons = document.querySelectorAll('.notif-filter');
    if (!container) return;

    // 1) Generate / read a stable device id (per browser on this site)
    const getDeviceId = () => {
        try {
            const params = new URLSearchParams(window.location.search);
            const fromUrl = params.get("deviceId") || params.get("mac") || params.get("macId");
            if (fromUrl) {
                localStorage.setItem("znu_device_id", fromUrl);
                return fromUrl;
            }
        } catch (e) {
            console.warn("URLSearchParams not available, skipping URL-based device id.");
        }

        try {
            let stored = localStorage.getItem("znu_device_id");
            if (!stored) {
                const randomPart = (window.crypto && window.crypto.randomUUID)
                    ? window.crypto.randomUUID()
                    : (Date.now().toString(36) + Math.random().toString(36).slice(2));
                stored = "dev-" + randomPart;
                localStorage.setItem("znu_device_id", stored);
            }
            return stored;
        } catch (e) {
            console.warn("localStorage not available, falling back to anonymous device id.");
    ...
    // 2) Local storage key for dismissed notifications per device
    const DISMISS_KEY = "znu_dismissed_notifications_" + DEVICE_ID;
    ...
    const loadDismissed = () => {
        try {
            ...
            console.warn("Could not read dismissed notifications from localStorage:", e);
            return [];
        }
    };
    ...
    const getNotificationKey = (item) => {
        // Try to use a backend id if it exists, otherwise build a stable key
        if (item.id) return String(item.id);
        const name = item.name || item.title || "noname";
        const updated = item.updatedTime || item.modifiedTime || "";
        const level = item.level || "";
        return name + "___" + updated + "___" + level;
    };

    let notifications = []; // data coming from the API
    let activeLevel = "all";
    ...
    const renderNotifications = () => {
        if (!notifications || notifications.length === 0) {
            ...
            container.innerHTML = `
                <div class="info-card notification-card">
                    <h3>مافيش إشعارات لسه</h3>
                    <p>أول ما يتحط ملفات جديدة هتظهر هنا كإشعارات.</p>
                </div>
            `;
            return;
        }
        ...
        let filtered = activeLevel === "all"
            ? notifications
            : notifications.filter(item => item.level === activeLevel);

        // Remove locally dismissed notifications for this device only
        filtered = filtered.filter(item => !dismissed.includes(getNotificationKey(item)));
        ...
        filtered.forEach(file => {
            const date = new Date(file.updatedTime || file.modifiedTime || Date.now());
            ...
            card.innerHTML = `
                <div class="notification-card-header">
                    ...
                    <div class="notification-card-text">
                        <h3>${file.name || file.title || "ملف بدون اسم"}</h3>
                        <p class="notification-meta">${levelLabel} • تحديث في: ${formatted}</p>
                    </div>
                </div>
                <div class="notification-actions">
                    <a href="${file.url || file.link || "#"}" target="_blank" rel="noopener noreferrer" class="general-button notification-open">
                        فتح الملف على Google Drive
                    </a>
                    <button type="button" class="general-button notification-dismiss">
                        إخفاء على هذا الجهاز
                    </button>
                </div>
            `;

            const dismissBtn = card.querySelector(".notification-dismiss");
            if (dismissBtn) {
                dismissBtn.addEventListener("click", (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    const key = getNotificationKey(file);
                    if (!dismissed.includes(key)) {
                        dismissed.push(key);
                        saveDismissed(dismissed);
                    }
                    // Re-render list without this notification
                    renderNotifications();
                });
            }
    ...
    // Fetch notifications from backend (Google Apps Script via Netlify)
    try {
        // If in the future you want the backend to also know the device,
        // you can append &deviceId=... here and handle it in Apps Script.
        const url = NOTIFICATIONS_API_URL; // + "&deviceId=" + encodeURIComponent(DEVICE_ID);
...
<!-- ================================== -->
<!-- START: VISITOR COUNTER via Netlify -> Google Apps Script -->
<!-- ================================== -->
<script>
document.addEventListener('DOMContentLoaded', async () => {
    const VISITOR_COUNTER_API_URL = "/.netlify/functions/visitor-counter?type=visitor";
    const counterElement = document.getElementById('visitor-count');
    if (!counterElement) return;

    try {
        const response = await fetch(VISITOR_COUNTER_API_URL);
        if (!response.ok) {
        ...
        } else if (data && typeof data.visits === "number") {
            counterElement.textContent = data.visits.toLocaleString("ar-EG");
        } else {
...
<!-- END: VISITOR COUNTER via Netlify -> Google Apps Script -->
